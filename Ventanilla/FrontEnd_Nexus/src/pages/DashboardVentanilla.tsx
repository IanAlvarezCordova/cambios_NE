...
const DashboardVentanilla = () => {
    ...
    // State para devoluciones
    const [devInstId, setDevInstId] = useState('');
    const [devCode, setDevCode] = useState('AC04');
    ...
    const handleOperar = async (e: React.FormEvent) => {
        ...
        // Si es devolucion, usamos otra logica
        if (tab === 'DEVOLUCION') {
            if (!devInstId || !devCode) return;
            setProcesando(true);
            try {
                await ventanillaService.solicitarDevolucion(devInstId, devCode, devDesc || "Solicitud Ventanilla");
                toast.success("Devolución solicitada correctamente");
                setDevInstId(''); setDevDesc('');
            } catch (error: any) {
                                    ...
                                    {!cuentaEsActiva ? (
                                        ...
                                        <>
                                            <div className="flex border-b overflow-x-auto">
                                                ...
                                                <button onClick={() => setTab('DEVOLUCION')} className={`flex-1 py-4 font-bold text-sm border-b-4 transition-all ${tab === 'DEVOLUCION' ? 'border-red-500 text-red-600 bg-red-50/30' : 'border-transparent text-gray-400 hover:bg-gray-50'}`}>DEVOLUCIÓN</button>
                                            </div>
                                                    ...
                                                    {tab === 'DEVOLUCION' ? (
                                                        <>
                                                            <div className="bg-red-50 p-4 rounded-xl border border-red-100 mb-4">
                                                                <p className="text-sm text-red-800 mb-2">Solicitar devolución de transacción (PACS.004)</p>
                                                            </div>
                                                            <div>
                                                                <label className="block text-xs font-bold text-gray-500 uppercase mb-2">Transaction / Instruction ID</label>
                                                                <input type="text" className="w-full p-4 border-2 border-gray-200 rounded-xl outline-none focus:border-red-500 font-mono" placeholder="Ej: TX-1234..." value={devInstId} onChange={e => setDevInstId(e.target.value)} required />
                                                            </div>
                                                            <div>
                                                                <label className="block text-xs font-bold text-gray-500 uppercase mb-2">Razón de Devolución</label>
                                                                <select className="w-full p-4 border-2 border-gray-200 rounded-xl outline-none focus:border-red-500" value={devCode} onChange={e => setDevCode(e.target.value)}>
                                                                    <option value="AC04">AC04 - Cuenta Cerrada</option>
                                                                    <option value="AM09">AM09 - Monto Incorrecto</option>
                                                                    <option value="AG01">AG01 - Transacción Prohibida</option>
                                                                    <option value="BE04">BE04 - Falta Dirección Acreedor</option>
                                                                    <option value="DS24">DS24 - Orden de Devolución (Cliente)</option>
                                                                    <option value="DUPL">DUPL - Duplicado</option>
                                                                </select>
                                                            </div>
                                                            <div>
                                                                <label className="block text-xs font-bold text-gray-500 uppercase mb-2">Descripción Adicional</label>
                                                                <input type="text" className="w-full p-4 border-2 border-gray-200 rounded-xl outline-none focus:border-red-500" placeholder="Detalle adicional..." value={devDesc} onChange={e => setDevDesc(e.target.value)} />
                                                            </div>
                                                            <button type="submit" disabled={procesando} className="w-full py-4 bg-red-600 text-white rounded-xl font-bold shadow-lg hover:bg-red-700 transition-all flex justify-center items-center gap-2 disabled:opacity-50">
                                                                {procesando ? <Loader2 className="animate-spin" /> : 'SOLICITAR DEVOLUCIÓN'}
                                                            </button>
                                                        </>
                                                    ) : (
                                                        // FORMULARIO NORMAL (DEPOSITOS, RETIROS, TRANSFERENCIAS)
                                                        <>
                                                            {tab === 'TRANSFERENCIA' && (
